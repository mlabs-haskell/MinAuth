...
describe('commitmentHexToField and commitmentFieldToHex Tests', () => {
  ...
  it('composition of commitmentFieldToHex and commitmentHexToField is identity', () => {
    const originalFieldInput: UserCommitmentField = {
      commitment: new Field(123)
    };
    const hexResult = commitmentFieldToHex(originalFieldInput);
    const fieldResult = commitmentHexToField(hexResult);
    expect(fieldResult).toEqual(originalFieldInput);
    expect(
      fieldResult.commitment.equals(originalFieldInput.commitment).toBoolean()
    ).toBe(true);
  });
  it('going via userCommitmentField and hex+parseToField generates the same Field', () => {
    const s1 = mkUserSecret({ secret: '0' });
    const sh1 = s1.secretHash;
    const f1 = Poseidon.hash([sh1]);
    const f2 = userCommitmentField(s1).commitment;
    expect(f1.toString()).toEqual(f2.toString());
    expect(f1.equals(f2).toBoolean()).toBe(true);
    const f3 = commitmentHexToField(
      commitmentFieldToHex({ commitment: f1 })
    ).commitment;
    expect(f1.equals(f3).toBoolean()).toBe(true);
  });
});
